...
const projectId = "2JlZf3Q6yrqPf28WN09cOv1N8Lj";
const projectSecretKey = "77605261642e2428ca66cddd5ec0d7c7";
const auth = `Basic ${Buffer.from(`${projectId}:${projectSecretKey}`).toString(
  "base64"
)}`;

const subdomain = "https://aoggle.infura-ipfs.io";
...
//     // const provider = new ethers.providers.Web3Provider(connection);
    const provider = new ethers.providers.JsonRpcProvider(
      "https://goerli.infura.io/v3/77605261642e2428ca66cddd5ec0d7c7"
    );
    const signer = provider.getSigner();
...
export const NFTMarketplaceProvider = ({ children }) => {
  ...
  const checkIfWalletConnected = async () => {
    try {
      if (!window.ethereum)
        return setOpenError(true), setError("Install MetaMask or Click Me and then Click Again on Homepage");
  ...
  const fetchNFTs = async () => {
    try {
      if (currentAccount) {
        const provider = new ethers.providers.JsonRpcProvider(
          "https://polygon-mumbai.g.alchemy.com/v2/KDFAi64pR0nHxqJGLR96juWLV00Dxyu3"
        );
